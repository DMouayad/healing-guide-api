...
export const facilityTypesRoutes = {
	baseRoute: "/facility-types",
	getAll: "",
	add: "",
	delete: (id = ":id") => `/${id}`,
	edit: (id = ":id") => `/${id}`,
...
router.get(facilityTypesRoutes.getAll, getAllAction);
router.post(facilityTypesRoutes.add, isAdmin, addAction);
router.patch(facilityTypesRoutes.edit(), isAdmin, updateAction);
router.delete(facilityTypesRoutes.delete(), isAdmin, deleteAction);

async function getAllAction(req: Request, res: Response) {
	const query = await commonZodSchemas.queryParams.parseAsync(req.query);
	return getAppCtx()
		.facilityTypeRepository.getAll(query)
		.then((items) => {
...
async function addAction(req: Request, res: Response) {
	...
	return getAppCtx()
		.facilityTypeRepository.store(data.name)
...
async function deleteAction(req: Request, res: Response) {
	...
	return getAppCtx()
		.facilityTypeRepository.delete(params.id)
...
async function updateAction(req: Request, res: Response) {
	...
	const params = await commonZodSchemas.requestIdParam.parseAsync(req.params);
	return getAppCtx()
		.facilityTypeRepository.update(params.id, data)
		.then((item) => ApiResponse.success().send(res));
}
